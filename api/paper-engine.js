...
const RISK_PER_TRADE = 0.01;             // 1% base; effective 0.5%-2% via star-mult
const BREAKEVEN_ATR = 1.0;
const TRAIL_ATR_BASE = 1.5;
const PARTIAL_PCT = 0.5;
const MAX_HOLD_HOURS = 120;
const MIN_RR = 0.5;
// Pool: 10 totaal (Merlijn 4 + Kronos 6). Env-overridable.
const MAX_POSITIONS = parseInt(process.env.MERLIJN_MAX_OPEN || '4', 10);
const MAX_CRYPTO_LONGS = 4;              // verlaagd: correlation guard tegen flash-crash (was 6)
const CASH_BUFFER_PCT = 0.25;            // houd ≥25% van startBalance vrij; effective max ~7-8 posities
const PORTFOLIO_KILL_DD_PCT = 0.05;      // 24h equity-drop > 5% → pauze ALLE entries (tightened per research, was 10%)
